#This file originates from https://github.com/ultralytics/yolov3.git
import glob
import math
import os
import random
...
def kmean_anchors(path='../coco/train2017.txt', n=9, img_size=(608, 608)):
    ...
    def print_results(k):
        ...
        for i, x in enumerate(k):
            ...
            print('%i,%i' % (round(x[0]), round(x[1])))  # use in *.cfg
        return k
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    tl = line_thickness or int(round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1)  # line thickness
    ...
    if label:
        ...
        return cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
